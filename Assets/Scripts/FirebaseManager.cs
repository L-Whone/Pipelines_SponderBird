...
public class FirebaseManager : MonoBehaviour
{
...
#else
    // if we don't find the external stuff
    // means we are running in editor so we send fake data
    ...
    public void OnAuthRecieved(string json)
    {
        ...
        IsAuthenticated = !string.IsNullOrEmpty(ProjectId) && !string.IsNullOrEmpty(IdToken);
    ...
    public void SubmitScore(int score, int pipes, int duration)
    {
        if(!IsAuthenticated)
        {
            Debug.Log("Not Authenticated. Score not submutted");
            return;
        }
